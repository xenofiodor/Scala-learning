...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def mostRetweeted: Tweet = {
    val withoutElem = this.remove(elem)
    try{
      val maxInSets = withoutElem.mostRetweeted
      if (maxInSets.retweets > elem.retweets) maxInSets
      else elem
    }
    catch{
...
object GoogleVsApple {
  ...
  def filterFunction(t: Tweet, list: List[String]): Boolean = {
    val size = list.length
    @scala.annotation.tailrec
    def goFrom(i: Int): Boolean = {
      if (i >= size) false
      else if (t.text.contains(list(i))) true
      else goFrom(i + 1)
    }
    goFrom(0)
  }
  ...
  lazy val googleTweets: TweetSet = allTweets.filter((t: Tweet) => filterFunction(t, google))
  lazy val appleTweets: TweetSet = allTweets.filter((t: Tweet) => filterFunction(t, apple))
